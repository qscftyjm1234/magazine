...
<template>
    ...
    <nav
        v-if="!isMobile()"
        class="bg-white w-full shadow-sm flex justify-center z-50" 
        ref="headerRef"
    ...
    >
        <div class="w-[1200px] flex items-center justify-between">
            ...
            >
                ...
                <!-- <svg class="h-8 w-8 text-gray-800 cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            >
            <!-- 依圖稿所放置 -->
            <div class="w-[327px] h-[28px] bg-gray-300 text-white flex items-center justify-center font-bold">
                327X28
            </div>
            <!-- search bar -->
        ...
        <div class="hidden w-full shadow-inherit absolute top-[64px] z-40" id="navbar-hamburger">
            <ul
    ...
    <!-- 移動端 -->
    <nav
        class="w-screen bg-white flex items-center justify-between fixed top-0 z-50 px-2 py-1 shadow-lg"
        v-else
    >
        <button
            data-collapse-toggle="navbar-hamburger"
            type="button"
            class="inline-flex items-center justify-center p-2 w-10 h-10 ml-3 text-sm text-gray-900 rounded-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200"
            aria-controls="navbar-hamburger"
            aria-expanded="false"
        >
            <!-- menu icon -->
        ...
        >
        <!-- logoIn -->
        <button type="button" class="inline-flex justify-center rounded-full bg-white px-3 py-2 text-sm font-semibold text-orange-400 shadow-sm ring-1 ring-inset ring-orange-400 hover:bg-gray-50 sm:mt-0 sm:w-auto">登入</button>
